...
def bar_chart(df, feature, label, roundto=4, p_threshold=0.05, sig_ttest_only=True):
    ...
    f, p = stats.f_oneway(
        *group_lists
    )  # <-- same as (group_lists[0], group_lists[1], ..., group_lists[n])
...
def crosstab(df, feature, label, roundto=4):
    ...
    ct_df = pd.DataFrame(
        np.rint(contingency_table).astype("int64"),
...
def bivariate(df, label, roundto=4):
    ...
    for feature in df:
        if feature != label:
            ...
            missing = (
                round((df.shape[0] - df_temp.shape[0]) / df.shape[0], roundto) * 100
            )
            ...
            if pd.api.types.is_numeric_dtype(
                ...
                df[feature]
            ) and not pd.api.types.is_numeric_dtype(df_temp[label]):
